class SasaDynamo {
  ...
  validateInput(value, type) {
      const regexPatterns = {
        ...
        url: /^(https?:\/\/)?([^\s.]+\.[^\s]{2,})$/, // Basic URL validation
        tel: /^[0-9\-\+\(\) ]{7,20}$/, // Phone number with basic characters
      ...
      if (type in regexPatterns) {
        return regexPatterns[type].test(value); // Validate against pattern
      }
      
      return true; // Default for 'text' and other undefined types
  }
  ...
  renderInitialInteraction(container, config) {
      const {
        ...
        buttonText = '',
        showWappCTA = true,
        inputType = 'text' // Nuevo parámetro con valor por defecto "text"
      ...
      const inputHTML = placeholder
        ? `<input type="${inputType}" class="form-control uk-form-large sasaUserInput uk-width-1-1@s uk-width-1-2@m uk-align-center" placeholder="${placeholder}">`
        : '';
...
async submitInitialData() {
    ...
    let userInput = '';
    const inputType = userInputElement ? userInputElement.getAttribute('type') : 'text';
    ...
    if (userInputElement && userInputElement.placeholder.trim() !== '') {
      ...
      if (!this.validateInput(userInput, inputType)) {
        alert(`Invalid ${inputType} format. Please enter a valid value.`);
        return;
      }
    }
